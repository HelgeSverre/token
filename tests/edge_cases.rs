...
fn test_set_cursor_from_position_past_end() {
    ...
    assert_eq!(model.editor().primary_cursor().line, 1);
    assert_eq!(model.editor().primary_cursor().column, 5); // End of "world"
}
...
fn test_insert_preserves_cursor_buffer_position_consistency() {
    ...
    for ch in "foo".chars() {
        let before_pos = model.document().cursor_to_offset(
        ...
        let after_pos = model.document().cursor_to_offset(
            model.editor().primary_cursor().line,
            model.editor().primary_cursor().column,
        );
...
fn test_multiple_inserts_middle_of_line_no_drift() {
    ...
    let initial_pos = model.document().cursor_to_offset(
        model.editor().primary_cursor().line,
        model.editor().primary_cursor().column,
    );
    ...
    for (i, ch) in insertions.chars().enumerate() {
        ...
        let actual_pos = model.document().cursor_to_offset(
            model.editor().primary_cursor().line,
            model.editor().primary_cursor().column,
        );
