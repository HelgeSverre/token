...
# (or `just profile-render --frames 500 --splits 3 --stats`, which builds with the release profile instead)
...
just profile-samply
# Or manually:
samply record ./target/profiling/token samples/large.txt   # generated by `just sample-large`
```

**xctrace / Instruments** (Apple Silicon friendly):

```bash
./target/profiling/token samples/large.txt &
APP_PID=$!
...
```bash
# DHAT heap profiler (writes dhat-heap.json and prints the dh_view URL to open it in)
just profile-memory

# Instruments Allocations
xcrun xctrace record --template "Allocations" --launch ./target/profiling/token -- samples/large.txt -o /tmp/alloc.trace
```
...
In debug builds, press F2 (or run **Toggle Performance Overlay** from the command palette) for a live performance overlay showing frame time, per-stage breakdown, glyph cache stats, and sparklines. Note: the overlay forces full redraws while visible, so it perturbs its own measurements.
...
just bench-multicursor   # Multi-cursor subset of the main loop bench
just bench-large         # Large-file benchmarks across all groups
just bench-completion    # Completion engine
just bench-wrap          # Soft-wrap layout
just profile-workloads   # End-to-end editor workloads (editor_workloads bench)
just bench-search        # Find/replace operations
just bench-layout        # Text measurement, viewport calculation
just bench-syntax        # Syntax highlighting (parser, highlight, expansion benches)
```

Individual bench files live in `benches/` (`completion`, `editor_workloads`, `glyph_cache`, `hot_paths`, `layout`, `main_loop`, `rendering`, `rope_operations`, `search`, `syntax`, `workspace`, `wrap`). Run a specific one:
...
The editor tracks 29 named render stages. In debug builds, `PerfStats` records per-stage timing histories (60 frames). In release builds, all perf code compiles away.
...
The editor's 29 internal render stages (`PerfStats`) can emit `tracing` spans visible to external profilers. This is controlled by Cargo feature flags and is off by default — zero overhead in normal builds.
...
- **`render_stage`** — nested spans for each of the 29 named stages, with a `stage` field like `text_glyphs`, `build_plan`, `sidebar`, `status_bar`, etc.
...
./target/release/token samples/large.txt
