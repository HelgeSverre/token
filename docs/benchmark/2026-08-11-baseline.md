...
Historical record, relocated on 2026-09-07 without changing its measurements.
It combines optimized search/syntax microbenchmarks, native startup experiments,
and explicitly debug-build syntax profiles from the earlier optimization work;
these are not one uniform benchmark run. References to “current” or “now” below
describe that historical work, not the present checkout.

For fresh optimized measurements and their limits, see the
[2026-09-07 current-state report](2026-09-07-current.md). The workloads differ;
do not calculate a before/after speedup between these reports. See the
[benchmark guide](README.md) for repeatable commands and measurement boundaries.
