...
- [x] Theme: new `overlay.*` keys, luminance-relative/minimum-contrast fallback
  resolution and the bundled-theme contrast checks are implemented. The original
  eight-theme tuning follow-up was completed 2026-09-08: Fleet Dark, GitHub
...
- [x] **`hit_test_modal` rewritten** against the shared layout; row-click activation, wheel scrolling, hover wash; the old inside-click drop removed. (Tabs are Phase 4 — no tab hit-test target exists yet.)
...
**Post-landing fix-up (adversarial review):** the first landing of this phase violated its own Hit-testing invariant — `with_modal_overlay_layout`'s `CommandPalette` arm built a shape-only spec with `tabs: None` and a flat, un-sectioned `state.matches` list regardless of `active_tab`, so the renderer (real tab bar + per-tab sections via a new `search_everywhere_sections` ordering-authority function) and the hit-tester/caret-placer disagreed on geometry: tab clicks fell through to `Modal { inside: true }` instead of `ModalTab`, row clicks activated the wrong `flat_index`, and the IME caret rect sat above the real input. Fixed by routing both consumers through `search_everywhere_sections` (`update::ui`, also now the source `commands_tab_shapes`/`all_tab_total` derive from) and `search_tab_bar`, with regression tests in `view::modal::tests` and `view::hit_test`. Also fixed in the same pass: `resolve_palette_rows` was duplicating the "Recently Used" top-3 immediately below their own section (the unfiltered list beneath it was independently recency-sorted); the Files tab's empty-results message reused the "no workspace" string even when a workspace was open; the All tab's count was hardcoded `Hidden`; `Unavailable` tabs rendered with the same dim color as a merely-inactive tab; and `automation.rs`'s All-tab snapshot duplicated the `5` cap as a literal instead of importing `update::ALL_TAB_GROUP_CAP`. Left as-is (judgment call): `ToggleModal`/`OpenCommandPalette` (`Cmd+Shift+A`) and `OpenFuzzyFileFinder` (`Cmd+Shift+O`) still force `active_tab` to `All`/`Files` respectively on open, per this doc's own Bindings rule — the "restore preserves ... active tab" State-merge clause has no code path that exercises it today, since both entry points are fixed-tab openers by design.

**Second post-landing fix-up (adversarial review):** `cycle_search_tab`/`activate_search_tab`/`build_file_finder_state` built the lazily-populated Files tab's `FileFinderState` with an empty query even though the header already showed a shared, non-empty one — the newly-populated tab silently showed unfiltered results for one keystroke until the next edit re-synced it. Fixed by threading the current query through a shared `seeded_file_finder_state` helper everywhere the Files index is built. Separately, the All view's Files group only ever appeared after the user had visited the Files tab at least once in the session (`state.files` was `None` on every All-tab-only entry point — `Cmd+Shift+A`, `ModalMsg::OpenCommandPalette`), making the merged view commands-only in practice; fixed by loading the file index eagerly wherever `active_tab` becomes `All` too, not just `Files` (same lazy/no-workspace posture, just triggered by either tab). Also fixed: `search_everywhere_sections`' All arm always emitted a `(Some("Commands"), 0)` section even with zero command matches, rendering a bare header with no rows and no empty-state message — now omitted when empty, with a "No matches" empty-state added alongside the existing Files/Symbols ones; ranking was recency-*major* (`Reverse(recency)` as the primary tiebreaker ahead of `Reverse(score)`), letting one stale execution permanently outrank a strictly better match — changed to a bounded flat boost added to the fuzzy score instead; `CommandHistory::version` was only ever set in tests, so `command-history.json` always persisted `"version": 0` — `record_execution`/`toggle_pin` now stamp `CURRENT_VERSION`; `'@'` prefix routing pinned the always-`Unavailable` Symbols tab (parking the user with only Backspace as an exit) — now checks `tab_available` like every other tab-switch path and falls through to a literal char insert otherwise; `command_history.rs`'s tests mutated the process-global `XDG_CONFIG_HOME` env var to point `load()`/`save()` at a scratch dir, racing every other test's `AppModel::new()` → `CommandHistory::load()` call under `cargo test`'s parallel threads — refactored into private `load_from`/`save_to(path)` so tests exercise real file I/O against an explicit tempdir path with no shared global state at all; and `automation.rs`'s Commands-tab snapshot hardcoded `section: None` for every row, so the "Recently Used" grouping wasn't observable outside the All tab — now tagged from `state.recent_count` like the real render does.
...
**Post-landing fix-up (adversarial review):** the first landing added `ui.cursor_overlay` and `handle_cursor_overlay_key` but never added it to `runtime/app.rs`'s `skip_keymap` — Up/Down/Enter/Tab are all `is_simple()` keymap bindings (`MoveCursorUp`/`MoveCursorDown`/`InsertNewline`/`InsertTab`), so the keymap dispatched and consumed them before `handle_key` (and therefore the cursor-overlay branch) was ever reached; only Escape worked. Fixed by adding `ui.cursor_overlay.is_some()` to `skip_keymap` alongside the modal/gesture/sidebar/terminal/csv checks. Also fixed: the advertised F9 binding for `CycleCursorOverlayDemo` was display-only (`CommandDef.keybinding` isn't consumed anywhere but the palette accessory) — added a real F9 arm in `runtime/app.rs` next to the F2/F7/F8 debug toggles, calling `update::execute_command`. Also fixed: wheel-scroll on `HoverRegion::CursorOverlay` grew `scroll` with `saturating_add` and no upper bound, so many downward notches required an equal number of upward notches before the (already fully-visible, `resolve_visible_window`-clamped) window would move again — now clamped to each kind's real `max_scroll` (0 for the 8-row/8-visible Completion demo; the static Hover zones card has no scroll concept). Also fixed: `resolve_panel_width`'s 200px cursor-anchor floor was applied *after* the `window_width - 32` margin clamp, so a window narrower than ~232px produced a popup wider than the window itself; now the floor is clamped again to `window_width` so the "clamped to window edges" invariant holds even in that degenerate case.

**Second post-landing fix-up (adversarial review):** the previous fix-up's `skip_keymap` entry over-corrected — it disabled the *entire* non-global keymap while a popup was open, not just the five claimed keys, so Backspace/Delete/plain Left-Right/shift-selection/Cmd+C/V/X/Z/A (all non-global `is_simple()` bindings) became silent no-ops and `Condition::OverlayRoutesKeys` was unreachable for any normal editor command. Fixed by moving the claim to a dedicated pre-keymap check: `runtime/app.rs` now calls `handle_cursor_overlay_key` (exposed `pub(crate)` from `runtime/input.rs`) before the keymap runs at all; it consumes exactly Up/Down/Enter/Esc/Tab and returns, otherwise the keymap runs normally (with `overlay_routes_keys` set in its context, as originally intended) and falls through to `handle_key` as usual — `cursor_overlay.is_some()` was removed from `skip_keymap`. Also fixed: `Anchor::Cursor`'s flip-above case anchored off the caret's *bottom* edge for both directions (the anchor point built in `view::modal::cursor_overlay_anchor` was always `rect.y + rect.h`), so a flipped popup's bottom edge sat just above the caret bottom while still covering the line being edited; `Anchor::Cursor` now carries the caret's top `y` and its line height `h` separately, so above uses `y - gap - panel_h` (clearing the real top of the line) and below uses `y + h + gap`. Also fixed: the hover demo used `prefer_below: true` though the Contexts table specifies Hover as "Cursor, above-preferred", and `row_count_for(DebugHover) == 0` made Up/Down consumed-but-inert instead of following the doc's "any keypress dismisses" rule for Hover — `handle_cursor_overlay_key` now dismisses on any key when `kind == DebugHover` before matching on specific keys, and the demo's anchor uses `prefer_below: false`.
