...
Completion resolving is narrower than “A's reply can never enrich A.” CompletionItemResolved carries document id, revision, a stable CandidateId (the menu item's id), detail, documentation, and additional edits. merge_resolved_item requires a live completion menu whose document_id and revision equal the reply and whose items still contain an item with that id; it merges into that item and marks the LSP item resolved, independent of filtered order or the selected row. It does **not** itself compare query text or resolve purpose. A valid late resolve can enrich the old underlying item if it still belongs to that valid menu, regardless of current selection; the side card is always projected from the _currently selected_ filtered row. Selecting B resets DocumentationState, so A cannot become B's card. Deferred acceptance is a separate post-merge path: only pending_resolve == Some(candidate) proceeds to accept; otherwise the docs-purpose reply only redraws.
...
| -------------- | ------------------------------------------------ | ------------------------- | ------------------------------------------------- | -------------------- |
| Completion     | selected menu item in current completion session | query-start text position | document, revision, candidate id                  | completion update    |
